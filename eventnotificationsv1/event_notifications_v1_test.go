...
var _ = Describe(`EventNotificationsV1`, func() {
		...
		Context(`Using mock server endpoint with timeout`, func() {
			BeforeEach(func() {
				testServer = httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
					...
					res.Header().Set("Content-type", "application/json")
					res.WriteHeader(202)
					fmt.Fprintf(res, "%s", `{"notification_id": "NotificationID", "email_attachments": [{"content": "Content", "filename": "Filename", "content_type": "ContentType", "disposition": "attachment"}]}`)
				}))
			})
	...
	Describe(`UpdateSandboxDestination(updateSandboxDestinationOptions *UpdateSandboxDestinationOptions) - Operation response error`, func() {
		updateSandboxDestinationPath := "/v1/instances/testString/destinations/testString/upgrade"
		Context(`Using mock server endpoint with invalid JSON response`, func() {
			BeforeEach(func() {
				testServer = httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
					defer GinkgoRecover()
					...
					// Verify the contents of the request
					Expect(req.URL.EscapedPath()).To(Equal(updateSandboxDestinationPath))
					Expect(req.Method).To(Equal("PATCH"))
					res.Header().Set("Content-type", "application/json")
					res.WriteHeader(200)
					fmt.Fprint(res, `} this is not valid json {`)
	...
	Describe(`Model constructor tests`, func() {
		Context(`Using a service client instance`, func() {
			...
			It(`Invoke NewCreateSourcesOptions successfully`, func() {
				...
				createSourcesOptionsEnabled := true
				createSourcesOptionsModel := eventNotificationsService.NewCreateSourcesOptions(instanceID, createSourcesOptionsName, createSourcesOptionsEnabled)
			...
			It(`Invoke NewListPreDefinedTemplatesOptions successfully`, func() {
				// Construct an instance of the ListPreDefinedTemplatesOptions model
				instanceID := "testString"
				listPreDefinedTemplatesOptionsModel := eventNotificationsService.NewListPreDefinedTemplatesOptions(instanceID)
			...
			It(`Invoke NewDestinationConfigOneOfPagerDutyDestinationConfig successfully`, func() {
				...
				_model, err := eventNotificationsService.NewDestinationConfigOneOfPagerDutyDestinationConfig(routingKey)
				Expect(_model).ToNot(BeNil())
				Expect(err).To(BeNil())
			})
